...
class PlasteringHelperView extends GetView<PlasteringHelperController> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: const Color(0xFFD9E4FC), appBar: AppBar(
      backgroundColor: const Color(0xFFD9E4FC), elevation: 0,
        leading: const BackButton(color: Colors.black),
        title: Text("${userList['firstName'] ?? ''} ${userList['lastName'] ?? ''}",
          style: const TextStyle(
            color: AppColors.textColor, fontWeight: FontWeight.w400, fontFamily: "poppins", fontSize: 15,),),
        centerTitle: true,),
      body: Column(
        ...
        children: [
          ...
          Expanded(child: ListView.builder(itemCount: dataList.length, itemBuilder: (context, index) {
                ...
                return InkWell(onTap: () {
                  ...
                  child: Container(margin: const EdgeInsets.symmetric(horizontal: 12, vertical: 8,
                  ),
                    ...
                    child: Row(
                      children: [Column(children: [
                            ClipRRect(borderRadius: BorderRadius.circular(8),
                              child: Image.network("https://jdapi.youthadda.co/${worker['userImg'] ?? ''}", height: 80, width: 80,
                                fit: BoxFit.cover,
                                errorBuilder: (_, __, ___) => Icon(Icons.person, size: 80,
                                      color: Colors.grey,),),),
                            const SizedBox(height: 6),
                            Row(children: [const Icon(Icons.star,
                                  color: Colors.amber, size: 14,),
                                const SizedBox(width: 2),
                                Text("${worker['rating'] ?? '4.7'} (${worker['reviewCount'] ?? '89'} reviews)",
                        ...
                        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start,
                            ...
                            children: [
                              Row(children: [Expanded(child: Text(
                                      "${worker['firstName'] ?? ''} ${worker['lastName'] ?? ''}", style: const TextStyle(fontSize: 14,
                                  ...
                                  Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4,),
                                    ...
                                    child: const Text(
                                      ...
                                      style: TextStyle(
                                        color: Colors.white,
                                        fontSize: 9, fontWeight: FontWeight.w500, fontFamily: "poppins",),),),],),
                              const SizedBox(height: 4),
                              // Availability
                              Row(children: [Icon(Icons.check_circle, color:
                                        worker['avail'] == 1
                                            ? Colors.green
                                            : Colors.red, size: 10,),
                                        ...
                                        text: TextSpan(
                                          ...
                                          children: [
                                            TextSpan(
                                              text: "Charge: ",
                                              style: TextStyle(
                                                fontSize: 10, color: AppColors.orage, fontWeight: FontWeight.w400,),),
                                            TextSpan(
                                              // text: "₹ ${worker['charge'] ?? '0'}/day",
                                              text: charge.toString(),
                                              style: TextStyle(fontSize: 11, color: AppColors.orage,
                                    ...
                                    onPressed: () {
                                      ...
                                      if (phoneNumber.isNotEmpty) {
                                        controller.makePhoneCall(
                                          worker['phone'] ?? '',
                                          worker,    // pass which item was tapped
                                        );
                                      } else {
                                        Get.snackbar(
                                          'Error',
                                ...
                                const SizedBox(width: 10),
                                  Expanded(
                                    child: SizedBox(
                                      height: 32,
                                      child: ElevatedButton(
                                        onPressed: () {final receiverId = worker['_id'].toString();
                                          final receiverName = '${worker['firstName'] ?? ''} ${worker['lastName'] ?? ''}'
                                                  .trim();
                                          final receiverImage = worker['userImg'] ?? '';
                                          final phoneNumber = worker['phone'] ?? '';
